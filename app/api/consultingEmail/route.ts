	...
	if (req.method === "POST") {
		try {
			...
			const mailOptionsCustomer = {
				...
				to: body?.email,
				subject:
					"Congratulations on booking your consulting session with BlueTech Consulting!",
				template: "consultingSuccessfulCustomer",
				context: {
			...
			const mailOptionsCompany = {
				...
				subject: `BlueTech CONSULTING Booking -- ${body?.firstName} ${body?.lastName} just booked a consulting session!`,
				template: "consultingSuccessfulCompany",
				context: {
					eventDate: body?.eventDate,
					startTime: body?.startTime,
					endTime: body?.endTime,
